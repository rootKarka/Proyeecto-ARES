...
export default function MisionTable({ misiones, onEdit, onDelete }) {
  return (
            ...
            {misiones.map((mision) => {
              ...
              return (
                <tr key={mision.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                  ...
                  <td className="p-4 text-gray-600 dark:text-gray-400">{mision.tipo.replace('_', ' ')}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-gray-500">{mision.robot ? `Robot #${mision.robot}` : "—"}</td>
                  <td className="p-4 text-gray-500">{formatDate(mision.fecha_inicio)}</td>
                  <td className="p-4">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => onEdit(mision)} className="p-1.5 text-gray-400 hover:text-blue-500">
                        <Pencil size={15} />
                      </button>
                      <button onClick={() => onDelete(mision)} className="p-1.5 text-gray-400 hover:text-red-500">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </td>
